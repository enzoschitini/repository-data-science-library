...
with col1:
    ...
    def main():
        with st.expander("Nuova Lezione"):
            ...
            ultimo_indice = df.tail(1)['Index'].values[0] + 1

            with col01:
                index = df.tail(1)['Index'].values[0] + 1 #st.number_input('Index', min_value=0, max_value=500, value=ultimo_indice)
                professoressa = st.text_input('Professoressa')
                ora = st.number_input('Ora', min_value=0, max_value=22, value=7)
            ...
            with col02:
                ...
                anno = st.number_input('Anno', min_value=2022, value=2024)
...
with col002:
    ...
    uts.distribuzione_delle_categorie(cosa_vuoi_vedere, df[df['Professoressa'] == professoressa_scelta], 'Bar')
...
def main():
    ...
    with st.expander("Analisi degli orari"):
        uts.distribuzione_delle_categorie('Ora', df, 'Bar')
        uts.distribuzione_delle_categorie('Giorno della settimana', df, 'Bar')
...
df = df[(df['Data'] >= data_iniziale) & (df['Data'] <= data_finale)]""" 